...
## ツリー構造の読み方 (Summary Tree)

`nfc-probe`, `nfc-dump`, `nfc-inspect` で表示される階層ツリーは以下の通りです。

- **Root/Area**: フォルダのように振る舞い、特定の範囲 (End Code まで) のサービスを包含します。
- **Service**: 実際にデータを持つエントリです。
- **KeyVer**: サービスのキーバージョン。`Request Service` で取得されます。
- **Attributes**: サービスの種類（bit定義に基づく）。
  - `(R/W)`: 読み書き可能 (Random)
  - `(R/O)`: 読み取り専用 (Random)
  - `[C]`: サイクリックサービス (書き込み時に B0 へシフト)
- **Blocks**: 読み取りに成功した、あるいは保持している有効なブロック数。

```text
FeliCa Service/Area Tree Structure
└── Root Area 0x0000 (End: 0xFFFE)
    ├── Area 0x1000 (End: 0x17FF)
    │   ├── 0x1008 (protected)          KeyVer: 0x0001    - blocks
    │   └── 0x100B (plain (R/W) [C])    KeyVer: 0x0000    - blocks
    └── Area 0x2000 (End: 0x27FF)
        ├── 0x2009 (plain (R/W))        KeyVer: 0x0000   32 blocks
```
